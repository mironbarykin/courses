...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if not person_mother and not person_father:
            ...
            if person_mother in two_genes:
                mother_probability = 1 - PROBS['mutation']
            elif person_mother in one_gene:
            ...
            elif person_father in one_gene:
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        gene_probabilties = sum(probabilities[person]['gene'].values())
        trait_probabilities = sum(probabilities[person]['trait'].values())
        
        new_gene_probabilities = dict()
        for genes, probability in probabilities[person]['gene'].items():
            new_gene_probabilities[genes] = probability / gene_probabilties
        new_trait_probabilities = dict()
        for trait, probability in probabilities[person]['trait'].items():
            new_trait_probabilities[trait] = probability / trait_probabilities
        
        probabilities[person]['gene'] = new_gene_probabilities
        probabilities[person]['trait'] = new_trait_probabilities
